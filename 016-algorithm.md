...
毎回このようなイテレーターのループをするfor文を書くのは間違いの元だ。ここで重要なのは、要素のそれぞれに対して`std::cout << *i ;`を実行するということだ。要素を先頭から末尾まで順番に処理するというのはライブラリにやってもらいたい。
...
`result`に代入されるのは関数`op`の戻り値だ。関数`op`は値を一つの引数で受け取り値を返す関数だ。
