...
`std::array<T,N>`は`T`型の要素を`N`個保持するクラスだ。この`<T,N>`についてはまだ学んでいないので、今回は`int`型を3個確保する。今までに学んだ要素だけで実装してみよう。
...
そして`operator []`を実装しよう。引数が`0`なら`m0`を、1ならm1を、2ならm2を返す。それ以外の値の場合、プログラムを強制的に終了させる標準ライブラリ、`std::abort`を呼び出す。
...
`std::array`にはまだ様々なメンバーがある。一つずつ順番に学んでいこう。
