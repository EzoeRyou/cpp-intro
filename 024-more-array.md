...
`const`をつけていないメンバー関数を`const`なクラスのオブジェクトから呼び出せない理由は、メンバー関数がデータメンバーを変更しない保証がないからだ。その保証をつけるのが`メンバー関数のconst修飾`だ。
...
そしてもう一つ重要なのは、`const`修飾されたメンバー関数がデータメンバーへのリファレンスを返す場合、
