...
次に`v`が破棄される。`v`のデストラクターは`w`のデストラクターと全く同じことをする。ただし、ポインター`first`の指すオブジェクトはすでにデストラクターが呼び出されているし、ポインター`first`の指す生のストレージも解放されている。
...
変数`w`はまず要素を保持するためのメモリを動的確保する。その後、`w`に`v`が代入されるわけだが、このとき`w`が動的確保したメモリを指すポインターの値が上書きされてしまう。`w`が破棄されるとき、`w`が元々持っていた要素は破棄されなくなり、ストレージも解放されなくなる。
