...
`-std=`はC++の規格を選択するオプションだ。C++17に準拠したいのであれば`-std=c++17`を指定する。読者が本書を読むころときに、C++20やあるいはもっと未来の規格が発行されていれば、最新のC++規格を選択するオプションを指定するべきだ。
...
どうだろうか。読者の環境にもよるが、知覚できるぐらいの時間がかかっているのではないだろうか。プログラミングの習得にはコードを書いてから実行までの時間は短い方がよい。そこで本格的にC++を学ぶ前に、コンパイル時間を短縮する方法を学ぶ。
